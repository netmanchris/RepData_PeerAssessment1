...
The mean of the total number of steps per day is `r round(mean_steps_per_day,0)`
...
The median of the total number of steps per day is `r round(median_steps_per_day,0)`
...
```
The interval located in row 104, which is interval `r max_steps[1]` contains contains the max-steps-value on average across all the days which is `r  round(max_steps[2])`
...
The number of missing values in the data set is `r number_na`
...
The median of steps taken is `r round(median_noNAs,0)`. 
